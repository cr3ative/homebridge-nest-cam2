...
class NexusStreamer extends EventEmitter {
  ...
  handleNexusPacket(type, payload) {
    ...
    case PacketType.PING:
      self.log.info('[NexusStreamer] Ping');
      break;
    case PacketType.OK:
      self.log.info('[NexusStreamer] OK');
      self.authorized = true;
      self._processPendingMessages();
      self.schedulePingMessage();
      break;
    case PacketType.ERROR:
      self.log.info('[NexusStreamer] Error');
      self.stopPlayback();
      break;
    case PacketType.PLAYBACK_BEGIN:
      self.log.info('[NexusStreamer] Playback Begin');
      self.handlePlaybackBegin(payload);
      break;
    case PacketType.PLAYBACK_END:
      self.log.info('[NexusStreamer] Playback End');
      break;
    case PacketType.PLAYBACK_PACKET:
      // self.log.info('[NexusStreamer] Playback Packet');
      self.handlePlaybackPacket(payload);
      break;
    case PacketType.LONG_PLAYBACK_PACKET:
      self.log.info('[NexusStreamer] Long Playback Packet');
      self.handlePlaybackPacket(payload);
      break;
    case PacketType.CLOCK_SYNC:
      self.log.info('[NexusStreamer] Clock Sync');
      break;
    case PacketType.REDIRECT:
      self.log.info('[NexusStreamer] Redirect');
      self.handleRedirect(payload);
      break;
    default:
      self.log.info('[NexusStreamer] Unhandled Type: ' + type);
    }
  }
  ...
  setupFFMPEGPipe(request) {
    ...
    if (sessionInfo) {
      ...
      let ffmpegCommand = '-use_wallclock_as_timestamps 1 -i - -c:v ' + self.ffmpegCodec + ' -an -pix_fmt yuv420p ' + x264Params + '-payload_type 99 -ssrc ' + videoSsrc + ' -f rtp -srtp_out_suite AES_CM_128_HMAC_SHA1_80 -srtp_out_params ' + videoKey.toString('base64') + ' rtsp://'+targetAddress+':' + targetVideoPort+'?rtcpport='+targetVideoPort+'&localrtcpport='+targetVideoPort+'&pkt_size=1316&reorder_queue_size=3000';
      ...
      ffmpeg.stderr.on('data', (data) => {
        self.log.info(`[ffmpeg stderr, ${targetVideoPort}] ${data}`);
      });
      self.ffmpeg = ffmpeg;
    }
  }
