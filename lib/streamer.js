...
class NexusStreamer extends EventEmitter {
  ...
  stopPlayback() {
    ...
    if (self.audioFfmpeg) {
      self.audioFfmpeg.kill('SIGKILL');
      self.audioFfmpeg = void 0;
    }
  ...
  handlePlaybackBegin(payload) {
    ...
    for (let i = 0; i < packet.channels.length; i++) {
      ...
      if (stream.codec_type === CodecType.H264) {
        ...
        self.log.info(stream.codec_type);
  ...
  handlePlaybackPacket(payload) {
    ...
    if (packet.channel_id === self.videoChannelID) {
      ...
      self.audioFfmpeg.stdin.write(Buffer.concat([Buffer.from([0x00, 0x00, 0x00, 0x01]), Buffer.from(packet.payload)]));
    } else if (packet.channel_id === self.audioChannelID) {
      if (!self.audioFfmpeg) {
        return;
      }
      self.audioFfmpeg.stdin.write(Buffer.from(packet.payload));
    }
  }
  ...
  setupFFMPEGPipe(request) {
    ...
    if (sessionInfo) {
      ...
      if (pathToFfmpeg) {
        ...
        audioFfmpeg = spawn(pathToFfmpeg, audioFfmpegCommand.split(' '), {env: process.env});
      } else {
        ffmpeg = spawn('ffmpeg', ffmpegCommand.split(' '), {env: process.env});
        audioFfmpeg = spawn('ffmpeg', audioFfmpegCommand.split(' '), {env: process.env});
      }
      ...
      //audio - https://github.com/KhaosT/homebridge-camera-ffmpeg/issues/9
      let audioFfmpegCommand = '-f s16le -i - -c:a libfdk_aac -profile:a aac_eld -vn -ac 1 -b:a ' + audioBitrate + 'k -flags:a +global_header -payload_type 110 -ssrc ' + audioSsrc + ' -f rtp -srtp_out_suite AES_CM_128_HMAC_SHA1_80 -srtp_out_params '+audioKey.toString('base64')+' srtp://'+targetAddress+':'+targetAudioPort+'?rtcpport='+targetAudioPort+'&localrtcpport='+targetAudioPort+'&pkt_size=376';
      audioFfmpeg.stdin.on('error', (e) => {
        if (e.code !== 'EPIPE') {
          self.log.error(e.code);
          self.stopPlayback();
        }
      });
      audioFfmpeg.stderr.on('data', (data) => {
        self.log.info(`${data}`);
      });
      self.audioFfmpeg = audioFfmpeg;
    }
  }
}
